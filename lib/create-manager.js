...
module.exports = {
  ...
  '(e.g. a PoolCluster from felixge\'s `mysql2` package).',
  ...
  inputs: {
    ...
    connectionString: {
      description: 'A connection string to use to connect to a mysql2 database.',
      extendedDescription: 'Be sure to include credentials.  You can also optionally provide the name of an existing database on your mysql2 server.',
      ...
      example: '===',
      // example: 'mysql2://mikermcneil:p4ssw02D@localhost:3306/some_db',
      required: true
    },
    ...
    meta: {
      ...
      description: 'Additional mysql2-specific options to use when connecting.',
  ...
  exits: {
    ...
    malformed: {
      description: 'The provided connection string is not valid for mysql2.',
      outputVariableName: 'report',
  ...
  fn: function createManager(inputs, exits) {
    ...
    // pools (see "PoolCluster": https://github.com/felixge/node-mysql2/blob/v2.10.2/Readme.md#poolcluster)
    ...
    // of additional mysql2 options that will be passed into `.createPool()`
    ...
    //  • https://github.com/felixge/node-mysql2#connection-options
    //
    ...
    if (!_.isUndefined(inputs.meta)) {
      ...
      // Use properties of `meta` directly as mysql2 client config.
    ...
    // Validate & parse connection string, pulling out mysql2 client config
    ...
    try {
      ...
      _e.message = util.format('Provided value (`%s`) is not a valid mysql2 connection string.', inputs.connectionString) + ' Error details: ' + _e.message;
    ...
    //
    // More about using pools with node-mysql2:
    //  • https://github.com/felixge/node-mysql2#pooling-connections
    var pool = felix.createPool(_mysqlClientConfig);

    // Bind an "error" handler in order to handle errors from connections in the pool,
    // or from the pool itself. Otherwise, without any further protection, if any mysql2
    // connections in the pool die, then the process would crash with an error.
    //
    // For more background, see:
    //  • https://github.com/felixge/node-mysql2/blob/v2.10.2/Readme.md#error-handling
    pool.on('error', function err(err) {
      ...
      if (!_.isUndefined(inputs.onUnexpectedFailure)) {
        inputs.onUnexpectedFailure(err || new Error('One or more pooled connections to mysql2 database were lost. Did the database server go offline?'));
      }
    });
